...
class TradingEnvironment:
    ...
    def step(self, action):
        ...
        if action == 1 and ema5_prev < ema8_prev and ema5_current > ema8_current and self.close_price[
            ...
            if price > self.entry[0] and self.side[0] == 1 and self.low_price[self.current_step] > self.stop_loss[0] and \
                    self.high_price[self.current_step] < self.take_profit[0]:
            ...
            elif price > self.entry[0] and self.side[0] == -1 and self.high_price[self.current_step] < self.stop_loss[
                0] and self.low_price[self.current_step] > self.take_profit[0]:
                self.winloss.append(-1)
                pct = abs((price - self.entry[0]) / self.entry[0])
                self.profits.append(
                    -(self.amount[0] * self.leverage[0] * pct) - (self.amount[0] * self.leverage[0] * 0.0005))
        ...
        return self.balance, done, self.winloss, sum(self.profits), self.action_list, sum(self.fit)
...
def test_model(network, test_data):
    ...
    while True:
        ...
        def signal2():
            if env.ema5[env.current_step - 1] < env.ema8[env.current_step - 1] and env.ema5[env.current_step] > env.ema8[env.current_step] and env.close_price[env.current_step] > env.open_price[env.current_step] and env.close_price[env.current_step] > env.ema100[env.current_step] and \
                    env.close_price[env.current_step - 1] < env.ema100[env.current_step - 1]:
                signal = 1
            elif env.ema5[env.current_step - 1] > env.ema8[env.current_step - 1] and env.ema5[env.current_step] < \
                    env.ema8[env.current_step] and env.close_price[env.current_step] < env.open_price[
                env.current_step] and env.close_price[env.current_step] < env.ema100[env.current_step] and env.close_price[env.current_step - 1] > env.ema100[env.current_step - 1]:
                signal = -1
            else:
                signal = 0
            return signal
        ...
        balance, done, win_loss, total_profits, action_list, fit = env.step(action)
        if done:
            break
...
if __name__ == "__main__":
    ...
    df_symbolused = df_symbol[:len(df_symbol)-2]
        ...
        try:
            ...
            while logic[-1]:
                try:
                    ...
                    # Rename columns for consistency
                    df_added.columns = ['symbol', 'open_symbolused', 'high_symbolused', 'low_symbolused', 'close_symbolused',
                                             'volume_symbolused']
                                ...
                                try:
                                    ...
                                    if len(positions) > 0:  # Adjusted to check for any filled positions
                                        for position in positions:
                                            ...
                                            take_profit_order = place_tp_in_buy(symbol_2, take_profit_price,position_size, marginmode)
                                ...
                                try:
                                    ...
                                    if len(positions) > 0:  # Adjusted to check for any filled positions
                                        for position in positions:
                                            ...
                                            take_profit_order = place_tp_in_sell(symbol_2, take_profit_price,position_size, marginmode)
                                            take_profit_algo_id = take_profit_order['data'][0]['algoId']
                                            tp_alg_id.append(take_profit_algo_id)
                                            print(f'Take_profit Order: {take_profit_order}')
                                        # Exit the position management loop after handling the position
